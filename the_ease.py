#!/usr/bin/env python3
"""kalshi-combo — THE-EASE COMBO strategy runner.
...
Usage:
    kalshi-combo                             # foreground dry-run: show combos from live data
    kalshi-combo --live                      # foreground LIVE: place real combo bets after gate
    kalshi-combo --bg                        # background daemon: auto-restarting
    kalshi-combo --bg --live                 # background LIVE daemon
    kalshi-combo --history                   # show running total across all days
    kalshi-combo --show-open                 # Kalshi balance + positions
"""
...
def print_game_slate(games: list[Game], category: str, live: bool):
    ...
    print()
    print(f"  {'='*70}")
    print(f"  {B}{WH}KALSHI-COMBO{R} — {mode}  |  {category}  |  {len(games)} games today")
    pst_now = datetime.now(timezone.utc) - timedelta(hours=7)
    print(f"  {DIM}{pst_now.strftime('%Y-%m-%d %H:%M PST')}{R}")
...
def print_ease_summary(tickets: list[ComboTicket]):
    ...
    print(f"  {'='*70}")
    print(f"  {B}{WH}KALSHI-COMBO — SUMMARY{R}")
    print(f"  {'='*70}")
    print()
...
def load_ease_state() -> dict:
    ...
    if not os.path.exists(STATE_FILE):
        return {"strategy": "KALSHI-COMBO", "runs": {}}
    try:
...
def daemon_loop(live: bool, poll_secs: int = 60) -> int:
    """Daemon mode: run PERPETUALLY — wait for 8 AM, poll until halftime gate,
    execute, then wait for tomorrow and repeat.  Never exits.
    ...
    while True:
        # Ensure bankroll exists before each daily cycle
        load_bankroll()
        ...
        if today_already_ran():
            ...
            while date.today().isoformat() == td:
                now = datetime.now()
                tomorrow_mid = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
                remaining = (tomorrow_mid - now).total_seconds()
                if remaining <= 0:
                    break
                chunk = min(remaining, 3600)
                print(f"  {DIM}Sleeping {int(chunk/60)}min until midnight…{R}")
                time.sleep(chunk)
            continue  # back to top — re-check today (now = tomorrow)
        ...
        fail_streak = 0
        while True:
            # Check if run was recorded (by a parallel process or earlier pass)
            if today_already_ran():
                state = load_ease_state()
            ...
            if ready:
                ...
                if result == 0:
                    print(f"  {DIM}Run complete.  Waiting for midnight.{R}\n")
                    break  # exit polling loop → outer loop
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="kalshi-combo",
        description="KALSHI-COMBO (THE-EASE) — 5-combo strategy with halftime gate (live Kalshi data only)",
    )
    ...
    if args.loop > 0 and not (live and ready):
        ...
        while True:
            ...
            if ready2:
                ...
                while date.today().isoformat() == td_today:
                    now = datetime.now()
                    mid = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
                    remaining = (mid - now).total_seconds()
                    if remaining <= 0:
                        break
                    time.sleep(min(remaining, 3600))
                print(f"\n  {GR}▶  NEW DAY{R} — {date.today().isoformat()} — restarting poll…\n")
                # Re-discover for the new day
                category, games, counts, got_data = discover_by_category()
                ready = True
                if not args.now:
                    ready, msg = halftime_gate(games)
        ...
        # Gate was already open or --now: run already completed above.
        # Stay alive perpetually — wait for tomorrow.
        td_today = date.today().isoformat()
        print(f"\n  {GR}✓ Today done{R} — waiting for midnight to repeat…\n")
        while True:
            while date.today().isoformat() == td_today:
                now = datetime.now()
                mid = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
                ...
                time.sleep(min(remaining, 3600))
            td_today = date.today().isoformat()
            print(f"\n  {GR}▶  NEW DAY{R} — {td_today} — re-running…\n")
            # Re-run for the new day
            category, games, counts, got_data = discover_by_category()
            live = args.live and got_data
            if len(games) < 4:
                print(f"  {YE}[WARN]{R} Only {len(games)} games — waiting for next cycle…")
                time.sleep(poll)
                continue
            ready = True
            if not args.now:
